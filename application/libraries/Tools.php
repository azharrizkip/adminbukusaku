...
class Tools
{
    function exportdata($tb,$title,$desc,$namafile,$folderpath='',$nawal,$nakhir,$tawal,$takhir)
    {
        //$nawal = '3103114254';
        //$nakhir = '3103114255';
        $CI=& get_instance();
       ...
       // $countfield=count($fields)-1;
        ...
        {
            $col+=1;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col,$space);
        }
        //$CI->db->select('nis,nama,jurusan,tpoin');
        //$result=$CI->db->get($table);

        if($tb == "user"){
          $result = $CI->db->query("select nis,nama,jurusan,tpoin,aksi from user where nis between '".$nawal."' and '".$nakhir."' order by nis asc");

        }else if($tb == "poin"){
          $result = $CI->db->query("select nis,nama,kelas,kode,jenis,pelanggaran,poin,tanggal from poin where tanggal between '".$tawal."' and '".$takhir."' and konfirmasi = 'BENAR' order by tanggal asc");
